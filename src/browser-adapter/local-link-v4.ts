/**
 * The browser operator link (ADR-0026). Identical transport, admission and URL rules to the v3
 * link it was copied from — the repository keeps successors as parallel files rather than
 * editing a frozen one — with the v4 identity and the wider tool profile.
 */
...
import {
  ...
  type BrowserOperatorToolName,
} from './protocol-v4.js';

export interface OperatorAdapterDiscovery {
  admissionUrl: string;
  bootstrapToken: string;
  protocolVersion: typeof BROWSER_OPERATOR_PROTOCOL_VERSION;
  adapterId: typeof BROWSER_OPERATOR_ADAPTER_ID;
}
...
export interface LocalOperatorAdapterLink {
  listTools(): Promise<readonly BrowserOperatorToolName[]>;
  call(request: BrowserOperatorAdapterRequest): Promise<BrowserOperatorAdapterResponse>;
  close(): Promise<void>;
}

const BROWSER_OPERATOR_TOOLS: readonly BrowserOperatorToolName[] = [
...
type ToolCallRequest = Extract<BrowserOperatorAdapterRequest, { type: 'tool.call' }>;

export class McpLocalOperatorAdapterLink implements LocalOperatorAdapterLink {
  #closed = false;
  private constructor(
  ...
  static async admit(discoveryValue: unknown, correlationId: string): Promise<McpLocalOperatorAdapterLink> {
    const discovery = parseOperatorAdapterDiscovery(discoveryValue);
    ...
    try {
      ...
      return new McpLocalOperatorAdapterLink(client, admitted.mcpUrl, admitted.bearerToken);
    } catch {
  ...
  async listTools(): Promise<readonly BrowserOperatorToolName[]> {
    this.#assertOpen();
    let actual: string[];
    try {
    ...
    const expected = [...BROWSER_OPERATOR_TOOLS].sort();
    if (actual.length !== expected.length || actual.some((name, index) => name !== expected[index])) {
      throw new Error('browser operator tool profile mismatch');
    }
    return [...BROWSER_OPERATOR_TOOLS];
  }

  async call(request: BrowserOperatorAdapterRequest): Promise<BrowserOperatorAdapterResponse> {
    this.#assertOpen();
    if (request.type !== 'tool.call') throw new Error('LocalOperatorAdapterLink requires tool.call request');
    try {
  ...
  #assertOpen(): void {
    if (this.#closed) throw new Error('LocalOperatorAdapterLink is closed');
  }
}

function localError(requestId: string): BrowserOperatorAdapterResponse {
  return parseBrowserOperatorResponse({
    version: BROWSER_OPERATOR_PROTOCOL_VERSION,
    type: 'error',
    requestId,
...
export function parseOperatorAdapterDiscovery(value: unknown): OperatorAdapterDiscovery {
  if (!value || typeof value !== 'object') throw new Error('Invalid browser operator adapter discovery');
  const record = value as Record<string, unknown>;
  if (Object.keys(record).sort().join(',') !== 'adapterId,admissionUrl,bootstrapToken,protocolVersion') {
    throw new Error('Invalid browser operator adapter discovery');
  }
  if (typeof record.admissionUrl !== 'string' || typeof record.bootstrapToken !== 'string') {
    throw new Error('Invalid browser operator adapter discovery');
  }
  if (record.protocolVersion !== BROWSER_OPERATOR_PROTOCOL_VERSION || record.adapterId !== BROWSER_OPERATOR_ADAPTER_ID) {
    throw new Error('Invalid browser operator adapter discovery');
  }
  ...
  catch { throw new Error('Invalid browser operator adapter discovery'); }
  return {
...
function parseLoopbackUrl(value: string, pathname: string): URL {
  ...
  catch { throw new Error('Invalid browser operator adapter URL'); }
  const loopback = url.hostname === '127.0.0.1' || url.hostname === '[::1]' || url.hostname === '::1';
  if (url.protocol !== 'http:' || !loopback || url.pathname !== pathname || url.search || url.hash || url.username || url.password) {
    throw new Error('Invalid browser operator adapter URL');
  }
  return url;
}
...
async function releaseBearer(mcpUrl: string, bearer: string): Promise<void> {
  try {
    ...
    await fetch(url, { method: 'POST', headers: { authorization: `Bearer ${bearer}` } });
  } catch {
