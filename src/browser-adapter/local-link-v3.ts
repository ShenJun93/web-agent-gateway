...
import {
  ...
  type BrowserVerifyToolName,
} from './protocol-v3.js';

export interface VerifyAdapterDiscovery {
  admissionUrl: string;
  bootstrapToken: string;
  protocolVersion: typeof BROWSER_VERIFY_PROTOCOL_VERSION;
  adapterId: typeof BROWSER_VERIFY_ADAPTER_ID;
}
...
export interface LocalVerifyAdapterLink {
  listTools(): Promise<readonly BrowserVerifyToolName[]>;
  call(request: BrowserVerifyAdapterRequest): Promise<BrowserVerifyAdapterResponse>;
  close(): Promise<void>;
}

const BROWSER_VERIFY_TOOLS: readonly BrowserVerifyToolName[] = [
  'health', 'workspace.open', 'repo.search', 'repo.snapshot', 'file.read',
  'verify.preview', 'verify.result',
];
type ToolCallRequest = Extract<BrowserVerifyAdapterRequest, { type: 'tool.call' }>;

export class McpLocalVerifyAdapterLink implements LocalVerifyAdapterLink {
  #closed = false;
  private constructor(
  ...
  static async admit(discoveryValue: unknown, correlationId: string): Promise<McpLocalVerifyAdapterLink> {
    const discovery = parseVerifyAdapterDiscovery(discoveryValue);
    ...
    try {
      ...
      return new McpLocalVerifyAdapterLink(client, admitted.mcpUrl, admitted.bearerToken);
    } catch {
  ...
  async listTools(): Promise<readonly BrowserVerifyToolName[]> {
    this.#assertOpen();
    let actual: string[];
    try {
    ...
    const expected = [...BROWSER_VERIFY_TOOLS].sort();
    if (actual.length !== expected.length || actual.some((name, index) => name !== expected[index])) {
      throw new Error('browser verify tool profile mismatch');
    }
    return [...BROWSER_VERIFY_TOOLS];
  }

  async call(request: BrowserVerifyAdapterRequest): Promise<BrowserVerifyAdapterResponse> {
    this.#assertOpen();
    if (request.type !== 'tool.call') throw new Error('LocalVerifyAdapterLink requires tool.call request');
    try {
  ...
  #assertOpen(): void {
    if (this.#closed) throw new Error('LocalVerifyAdapterLink is closed');
  }
}

function localError(requestId: string): BrowserVerifyAdapterResponse {
  return parseBrowserVerifyResponse({
    version: BROWSER_VERIFY_PROTOCOL_VERSION,
    type: 'error',
    requestId,
...
export function parseVerifyAdapterDiscovery(value: unknown): VerifyAdapterDiscovery {
  if (!value || typeof value !== 'object') throw new Error('Invalid browser verify adapter discovery');
  const record = value as Record<string, unknown>;
  if (Object.keys(record).sort().join(',') !== 'adapterId,admissionUrl,bootstrapToken,protocolVersion') {
    throw new Error('Invalid browser verify adapter discovery');
  }
  if (typeof record.admissionUrl !== 'string' || typeof record.bootstrapToken !== 'string') {
    throw new Error('Invalid browser verify adapter discovery');
  }
  if (record.protocolVersion !== BROWSER_VERIFY_PROTOCOL_VERSION || record.adapterId !== BROWSER_VERIFY_ADAPTER_ID) {
    throw new Error('Invalid browser verify adapter discovery');
  }
  ...
  catch { throw new Error('Invalid browser verify adapter discovery'); }
  return {
