  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/') {
        return html(res, renderList(options.coordinator.listPendingLocal(20), session.csrf));
      }
      const detail = /^\/mutations\/([^/]+)$/.exec(url.pathname);
      if (req.method === 'GET' && detail) {
        const review = options.coordinator.reviewLocal(decodeURIComponent(detail[1]!));
        if (!review) return deny(res, 404);
        return html(res, renderReview(review, session.csrf));
      }
      const action = /^\/mutations\/([^/]+)\/(approve|reject)$/.exec(url.pathname);
      if (req.method === 'POST' && action) {
        if (req.headers.origin !== origin) return deny(res, 403);
        const form = await readForm(req);
        const csrf = form.get('csrf');
        if (!csrf || !sameSecret(csrf, session.csrf)) return deny(res, 403);
        const mutationId = decodeURIComponent(action[1]!);
        const ok = action[2] === 'approve'
...
function renderList(reviews: MutationLocalReviewView[], csrf: string): string {
  const items = reviews.map((review) => renderReview(review, csrf)).join('');
  return `<!doctype html><meta charset="utf-8"><title>WAG Review</title><h1>Pending mutations</h1>${items || '<p>None</p>'}`;
}

function renderReview(review: MutationLocalReviewView, csrf: string): string {
  ...
  return `<article><h2>${escapeHtml(review.path)}</h2>`
    ...
    + actionForm(actionId, 'approve', csrf)
    + actionForm(actionId, 'reject', csrf)
    + `</article>`;
}

function actionForm(id: string, action: 'approve' | 'reject', csrf: string): string {
  return `<form method="post" action="/mutations/${id}/${action}">`
