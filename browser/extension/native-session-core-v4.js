const BROWSER_OPERATOR_ADAPTER_ID = 'browser.chatgpt.native.operator.v4';
const BROWSER_OPERATOR_PROTOCOL_VERSION = 4;
...
export function createNativeOperatorSessionController({ connectNative, randomUUID, onToolResponse }) {
  let port = null;
  ...
  function handleMessage(message) {
    ...
    if (typeof requestId === 'string' && pendingControl.has(requestId)) {
      ...
      if (message.version !== BROWSER_OPERATOR_PROTOCOL_VERSION) {
        reject(new Error(`Invalid protocol version: expected ${BROWSER_OPERATOR_PROTOCOL_VERSION}`));
  ...
  function handleDisconnect() {
    ...
    for (const { reject } of errors) reject(new Error('Native messaging host disconnected'));
  }
  ...
  async function ensureReady(sessionId) {
    if (port && helloVerified && boundSession === sessionId) return;
    ...
    const handshake = (async () => {
      if (!helloVerified) {
        ...
        if (
          result?.protocolVersion !== BROWSER_OPERATOR_PROTOCOL_VERSION ||
          result?.adapterId !== BROWSER_OPERATOR_ADAPTER_ID
      ...
      if (boundSession !== sessionId) {
        ...
        if (!validateTools(listResult?.tools)) throw new Error('Native host tools profile mismatch');
  ...
  function postTool(request) {
    ...
    if (request.version !== BROWSER_OPERATOR_PROTOCOL_VERSION) throw new Error('Invalid verify protocol request');
  ...
  return { ensureReady, postTool, isConnected };
}
