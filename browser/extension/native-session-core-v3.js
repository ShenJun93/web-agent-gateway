const BROWSER_VERIFY_ADAPTER_ID = 'browser.chatgpt.native.verify.v3';
const BROWSER_VERIFY_PROTOCOL_VERSION = 3;
...
export function createNativeVerifySessionController({ connectNative, randomUUID, onToolResponse }) {
  let port = null;
  ...
  function handleMessage(message) {
    ...
    if (typeof requestId === 'string' && pendingControl.has(requestId)) {
      ...
      if (message.version !== BROWSER_VERIFY_PROTOCOL_VERSION) {
        reject(new Error(`Invalid protocol version: expected ${BROWSER_VERIFY_PROTOCOL_VERSION}`));
  ...
  async function ensureReady(sessionId) {
    ...
    const handshake = (async () => {
      if (!helloVerified) {
        ...
        if (
          result?.protocolVersion !== BROWSER_VERIFY_PROTOCOL_VERSION ||
          result?.adapterId !== BROWSER_VERIFY_ADAPTER_ID
  ...
  function postTool(request) {
    ...
    if (request.version !== BROWSER_VERIFY_PROTOCOL_VERSION) throw new Error('Invalid verify protocol request');
