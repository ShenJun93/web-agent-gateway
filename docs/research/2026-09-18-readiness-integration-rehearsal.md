...
## Browser-fix-first topology rehearsal — 2026-09-18 14:23 +07:00

The isolated Browser local-gate repair was rehearsed as a standalone three-file commit on exact `e19d577`:
...
A second temporary clone then rebased all 25 readiness commits from `e19d577..f880ce6` onto that hypothetical Browser-fix commit.
...
The final rebased tree is byte-identical to current readiness, so extracting the Browser-owned fixes first does not require changing readiness content; it only changes commit ancestry.

Live reachability review before recording this rehearsal:
- `f880ce6` is contained only by local branch `docs/signpath-foundation-readiness-v1`;
- `e296da1` is also contained only by that local branch;
- no tag contains the readiness head;
- neither `feat/browser-inspect-v2` nor `docs/signpath-foundation-readiness-v1` exists on the remote;
- each branch has one dedicated local worktree.

This makes the clean future sequence technically viable if separately authorized:

1. apply and commit only the three Browser local-gate fixes on `feat/browser-inspect-v2`;
2. rerun the exact Browser local gate;
3. explicitly authorize rewriting the local-only readiness ancestry;
4. rebase the 25 readiness commits onto the accepted Browser-fix commit;
5. prove final tree equality and rerun readiness verification;
6. only then consider any later push/PR/merge/publication/version/provider/signing gate.

This rehearsal does not authorize any of those mutations. No live branch was rebased, reset, pushed, merged, tagged, published, versioned, submitted, or signed.
